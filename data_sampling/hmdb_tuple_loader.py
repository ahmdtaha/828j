...
class HMDBTupleLoader:
    ...
    def load_subset(self,subset):
        df = pd.read_pickle(config.hmdb_session_path + '/'+subset+'_db.pkl')
        videos_names = df['video-name'].tolist()
        videos_lbls = df['video-lbl'].tolist()
        videos_len = df['video-len'].tolist()
    ...
    def get_context(self,center_idx,session,ordered=True):
        ...
        for idx in [-3,-2,-1,1,2,3]:
            img_path = os.path.join(config.hmdb_session_path, session, 'frame_%04d.jpg' % (center_idx+idx));
    ...
    def get_img(self,frame_idx,session):
        ...
        img_path = os.path.join(config.hmdb_session_path,session,'frame_%04d.jpg' % frame_idx);
        img = imageio.imread(img_path);
