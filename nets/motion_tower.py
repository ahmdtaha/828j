...
class MotionTower:
    ...
    def __init__(self, mode = tf.estimator.ModeKeys.TRAIN,train_motion_tower=True,supervised=True):
        ...
        with tf.variable_scope("supervised_fc") as scope:
            fc6_dropout = tf.layers.dropout(inputs=self.fc6_context_dense, rate=0.85, training=mode == tf.estimator.ModeKeys.TRAIN)
            supervised_fc7 = tf.layers.dense(inputs=fc6_dropout, units=fcn_num_units, name='fc7', activation=tf.nn.relu,trainable=True);
            fc7_dropout = tf.layers.dropout(inputs=supervised_fc7 , rate=0.85,training=mode == tf.estimator.ModeKeys.TRAIN)
            supervised_logits = tf.layers.dense(inputs=fc7_dropout , units=num_classes, name='logits',trainable=True)
