...
class TwoStreamRegNet:
    ...
    *prefix*.  Try removing the '.' and extension.  Try:
    ...
    def __init__(self,supervised=False,train_spatial_tower=False,train_motion_tower=True,load_alex_weights=False):
        ...
        self.unsupervised_regs = tf.placeholder(tf.int32, shape=(batch_size),
                                                  name='gt_regs')
        ...
        if(supervised):
            ...
            with tf.variable_scope("unsupervised_fc") as scope:
                ...
                # unsupervised_logits = tf.layers.dense(inputs=unsupervised_fc8 , units=2, name='logits',trainable=not supervised) ## Belong to same video or not
                self.unsupervised_regression = tf.layers.dense(inputs=unsupervised_fc8 , units=1, name='regression', trainable=not supervised)  ## Belong to same video or not

            # with tf.variable_scope("unsupervised_logits") as scope:
            ...
            #     self.lbl_loss = tf.reduce_mean(cross_entropy, name='xentropy_mean')

            with tf.variable_scope("unsupervised_reg") as scope:
                reg_loss = tf.losses.mean_squared_error(self.unsupervised_regs,self.unsupervised_regression)
                self.reg_loss = tf.reduce_mean(reg_loss, name='reg_loss')
            ...
            #self.loss = self.lbl_loss + self.reg_loss;
