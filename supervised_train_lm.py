...
sys.path.append('data_sampling/')

# from data_sampling.tuple_loader import TupleLoader
from tuple_loader import TupleLoader
...
def gen_feed_dict(model, data_generator, subset, fix, args):
    if args[data_args.gen_nearby_frame]:
        words, nearby, context, lbl = data_generator.next(subset, fix,
                                                          supervised=True)
        feed_dict = {model.input_words: words, model.nearby_words: nearby,
                     model.input_context: context, model.supervised_labels: lbl}
    else:
        words, context, lbl = data_generator.next(subset, fix, supervised=True)
        feed_dict = {model.input_words: words, model.input_context: context,
                     model.supervised_labels: lbl}
...
if __name__ == '__main__':
    ...
    # TODO: @Moustafa: replace last_step with global step
    ...
    for step in range(last_step, const.train_iters):
        ...
        feed_dict = gen_feed_dict(img2vec_model, img_generator,
                                  const.Subset.TRAIN, None, args)
        model_loss_value, accuracy_value, _ = sess.run(
            [model_loss, model_accuracy, train_op], feed_dict)
        ...
        if step % const.logging_threshold == 0:
            ...
            if step != 0:
                ...
                feed_dict = gen_feed_dict(img2vec_model, img_generator,
                                          const.Subset.TRAIN, None, args)
                train_loss_op, _ = sess.run(
                    [train_loss, train_op], feed_dict=feed_dict)

                feed_dict = gen_feed_dict(img2vec_model, img_generator,
                                          const.Subset.VAL, None, args)
                val_loss_op, accuracy_op = sess.run([val_loss, model_acc_op],
                                                    feed_dict=feed_dict)

                # if(step % 1000 == 0):
                #     # Inspect true positive (TP), FP, TN, TP per class
                #     val_acc = np.zeros((file_const.num_classes,
                #                         file_const.num_classes))
                #     for class_i in range(file_const.num_classes):
                #         feed_dict = gen_feed_dict(
                #             img2vec_model, img_generator, const.Subset.VAL,
                #             class_i, args)
                #         prediction = sess.run(img2vec_model.class_prediction,
                #                               feed_dict=feed_dict)
                #         # bins = np.bincount(prediction,minlength=10)
                #         bins = np.histogram(prediction, np.arange(
                #             0, file_const.num_classes+1, 1))[0]
